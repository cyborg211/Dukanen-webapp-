...
export default async function ProductPage({params}:{params:{slug:string}}){
  ...
  const {data,error}=await supabase
    .from('products')
    .select('id,title,slug,description,price,currency,condition,city,country,status,negotiable,category_id,sellers(id,store_name,verified,user_id),categories(name),product_images(image_url,sort_order)')
    .eq('slug',params.slug)
    .eq('status','active')
    .maybeSingle();
  ...
  if(product.categoryId){
    const {data:relatedRows}=await supabase
      .from('products')
      .select('id,title,slug,price,currency,condition,city,country,sellers(store_name),product_images(image_url,sort_order)')
      .eq('status','active')
    ...
    related=(relatedRows||[]).map((p:any)=>{
      ...
      return {slug:p.slug||p.id,title:p.title,price:`${p.currency||'SSP'} ${Number(p.price).toLocaleString()}`,condition:p.condition||'Available',location:[p.city,p.country].filter(Boolean).join(', ')||'South Sudan',seller:p.sellers?.store_name||'Dukanen Seller',image:imgs[0]?.image_url,emoji:'🛍️'};
    });
  }
    ...
    <div className="container product-detail product-detail-v2">
      <section className="product-gallery" aria-label="Product photos">
        ...
        <div className="product-thumbs" aria-label="Photo thumbnails">
          {product.images.length?product.images.slice(0,6).map((src,index)=><div className={`product-thumb ${index===0?'active':''}`} key={src}><img src={src} alt={`${product.title} photo ${index+1}`}/></div>):<span className="product-thumb placeholder" aria-label="No additional photos">＋</span>}
        </div>
      </section>
      ...
      <section className="detail-copy product-info-panel">
        ...
        <div className="product-title-actions"><h1>{product.title}</h1><form action={toggleFavorite}><input type="hidden" name="productId" value={product.id}/><input type="hidden" name="slug" value={product.slug}/><button type="submit" className={`favorite-button ${isFavorite?'saved':''}`} aria-label={isFavorite?'Remove from favorites':'Save to favorites'}>{isFavorite?'♥':'♡'}</button></form></div>
        <div className="price">{product.price}</div>
        <div className="product-meta"><span>📍 {product.location}</span><span aria-hidden="true">•</span><span>{product.category}</span></div>
        ...
        <div className="contact-actions">
          ...
          <form action={startConversation}><input type="hidden" name="productId" value={product.id}/><input type="hidden" name="sellerId" value={product.sellerId}/><input type="hidden" name="slug" value={product.slug}/><input type="hidden" name="intent" value="call"/><button className="secondary" type="submit">☎ Contact seller</button></form>
        </div>
        {product.negotiable&&<form action={startConversation}><input type="hidden" name="productId" value={product.id}/><input type="hidden" name="sellerId" value={product.sellerId}/><input type="hidden" name="slug" value={product.slug}/><input type="hidden" name="intent" value="offer"/><button className="offer-button" type="submit">Make an offer</button></form>}
        ...
        <div className="safety-card"><div className="safety-icon" aria-hidden="true">✓</div><div><h3>Trade safely on Dukanen</h3><p>Meet in a public place, inspect the item before paying, and avoid sending money before you have verified the seller and product.</p></div></div>
        <Link className="report-link" href={`/messages?report=${encodeURIComponent(product.slug)}`}>⚑ Report this listing</Link>
      </section>
    </div>

    {related.length>0&&<section className="related-section"><div className="container"><div className="section-head"><div><h2>Similar listings</h2><p>More live listings in the same category.</p></div><Link href={`/marketplace?category=${encodeURIComponent(product.category)}`}>See all →</Link></div><div className="product-grid">{related.map((p:any)=><ProductCard key={p.slug} product={p}/>)}</div></div></section>}
  </div>;
}
